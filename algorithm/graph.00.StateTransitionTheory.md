...
graph TD
    ...
    A -. 正交维度 .-> UF["并查集 Union-Find<br/>集合代数结构"]
    UF --> UF1["纯连通性判定/分量计数"]
    UF --> UF2["动态增量合并（在线算法）"]
    UF --> UF3["环检测与冗余边定位"]
```
...
### 正交维度：并查集 (Union-Find) — 集合代数

并查集不在上述「有环/无环」的分水岭分类树上。它是一个**正交维度**：不是遍历算法，而是一种**集合管理的代数结构**。

* **数学本质**：等价关系 (Equivalence Relation) 的运行时实现。当问题中的关系满足自反、对称、传递三性质时，并查集天然适用。
* **与 DFS/BFS 的对偶**：DFS/BFS 是「自顶向下的过程性遍历」，并查集是「自底向上的声明性代数运算」。两者在连通性问题上计算的都是等价关系的传递闭包，但并查集在**动态增量合并**场景下具有 O(α(n)) ≈ O(1) 的时间优势。
* **不擅长**：路径还原、距离计算、有向图连通性。这些仍是 DFS/BFS 的领地。
* **相关文档**：[并查集](./unionFindSet.md)
...
4. **并查集 (Union-Find)** 作为正交的集合代数维度，提供连通性问题的声明式解法，尤其在动态增量场景下具有不可替代的效率优势。
